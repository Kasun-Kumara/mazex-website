...
const functionDir = resolve(rootDir, "functions/sync-registration-to-google-sheets");
const functionEnvPath = resolve(functionDir, ".env");
const archivePath = resolve(rootDir, ".tmp-registration-google-sheets-sync.tar.gz");
...
const functionId = "registration_google_sheets_sync";
const functionName = "Registration Google Sheets Sync";
const entrypoint = "src/main.js";
const commands = "npm install --omit=dev";
const events = ["databases.*.collections.*.documents.*.create"];
const scopes = [Scopes.DatabasesRead, Scopes.DocumentsRead];
...
async function syncVariables() {
  ...
  for (const [key, value] of Object.entries(functionEnv)) {
    ...
    const secret = key === "GOOGLE_OAUTH_CLIENT_SECRET";
...
async function deployArchive() {
  const deployment = await functions.createDeployment({
    functionId,
    code: InputFile.fromPath(archivePath, "registration-google-sheets-sync.tar.gz"),
    activate: true,
    entrypoint,
    commands,
  });
...
console.log("Google Sheets sync function deployment request submitted.");
